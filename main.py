...
@bot.on_message(filters.command(["start"]))
async def send_start(client: pyrogram.client.Client, message: pyrogram.types.messages_and_media.message.Message):
    await bot.send_message(message.chat.id, f"**__👋 Hi** **{message.from_user.mention}**, **I am Save Restricted Bot, I can send you public restricted content by post link note : only public__**\n\n{USAGE}",
...
async def save(client: pyrogram.client.Client, message: pyrogram.types.messages_and_media.message.Message):
    ...
    if "https://t.me/+" in message.text or "https://t.me/joinchat/" in message.text:
        try:
            await bot.send_message(message.chat.id, f"**Chat Joined**", reply_to_message_id=message.id)
        except UserAlreadyParticipant:
    ...
    # getting message
        ...
        for msgid in range(fromID, toID+1):
            ...
            # public
                ...
                username = datas[3]
                try: msg = await bot.get_messages(username, msgid)
                except UsernameNotOccupied: 
                    await bot.send_message(message.chat.id, f"**The username is not occupied by anyone**", reply_to_message_id=message.id)
                    return

                try: await bot.copy_message(message.chat.id, msg.chat.id, msg.id, reply_to_message_id=message.id)
                except:
                    try: await handle_private(message, username, msgid)
                    except Exception as e: await bot.send_message(message.chat.id, f"**Error** : __{e}__", reply_to_message_id=message.id)
...
async def handle_private(message: pyrogram.types.messages_and_media.message.Message, chatid: int, msgid: int):
    msg: pyrogram.types.messages_and_media.message.Message = await bot.get_messages(chatid, msgid)
    msg_type = get_message_type(msg)
